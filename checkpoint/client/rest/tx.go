...
func repairCheckpointHandler(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			...
			txBroadcaster2 := broadcaster.NewTxBroadcaster(cdc)
...
// repairCheckpointHandlerFixed 修复版本的checkpoint修复处理器
func repairCheckpointHandlerFixed(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 记录开始操作的日志
		helper.Logger.Info("repairCheckpointHandlerFixed, 开始处理补录消息",
			"checkpointNumber", req.CheckpointNumber,
			"from", from.String(),
		)
		...
		// 验证消息
		if err := msg.ValidateBasic(); err != nil {
			helper.Logger.Error("repairCheckpointHandlerFixed, 消息验证失败", "error", err)
			rest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
			return
		}

		// 直接广播交易，而不是生成未签名交易
		helper.Logger.Info("repairCheckpointHandlerFixed, 直接广播交易",
			"checkpointNumber", req.CheckpointNumber,
			"from", from.String(),
		)

		// 使用 TxBroadcaster 直接广播
		txBroadcaster := broadcaster.NewTxBroadcaster(cdc)
		err := txBroadcaster.BroadcastToHeimdall(msg)
		if err != nil {
			helper.Logger.Error("repairCheckpointHandlerFixed, 直接广播失败", "error", err)
...
// repairCheckpointTestHandlerFixed 修复版本的测试处理器
func repairCheckpointTestHandlerFixed(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 记录开始处理的日志
		helper.Logger.Info("repairCheckpointTestHandlerFixed, 开始处理测试消息",
			"checkpointNumber", req.CheckpointNumber,
			"testMessage", req.TestMessage,
			"from", from.String(),
		)
		...
		// 验证消息
		if err := msg.ValidateBasic(); err != nil {
			helper.Logger.Error("repairCheckpointTestHandlerFixed, 消息验证失败", "error", err)
			rest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
			return
		}

		// 直接广播交易，而不是生成未签名交易
		helper.Logger.Info("repairCheckpointTestHandlerFixed, 直接广播交易",
			"checkpointNumber", req.CheckpointNumber,
			"testMessage", req.TestMessage,
			"from", from.String(),
		)
		...
		err := txBroadcaster.BroadcastToHeimdall(msg)
		if err != nil {
			helper.Logger.Error("repairCheckpointTestHandlerFixed, 直接广播失败", "error", err)
			rest.WriteErrorResponse(w, http.StatusInternalServerError, err.Error())
			return
		}
...
// repairCheckpointHandlerOptimized 优化版本的checkpoint修复处理器
func repairCheckpointHandlerOptimized(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 记录开始操作的日志
		helper.Logger.Info("repairCheckpointHandlerOptimized, 开始处理补录消息",
			"checkpointNumber", req.CheckpointNumber,
			"from", from.String(),
		)
		...
		// 验证消息
		if err := msg.ValidateBasic(); err != nil {
			helper.Logger.Error("repairCheckpointHandlerOptimized, 消息验证失败", "error", err)
			rest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
			return
		}

		// 使用标准的 REST 方式生成未签名交易
		helper.Logger.Info("repairCheckpointHandlerOptimized, 生成未签名交易",
			"checkpointNumber", req.CheckpointNumber,
			"from", from.String(),
		)
...
// repairCheckpointTestHandlerOptimized 优化版本的测试处理器
func repairCheckpointTestHandlerOptimized(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 记录开始处理的日志
		helper.Logger.Info("repairCheckpointTestHandlerOptimized, 开始处理测试消息",
			"checkpointNumber", req.CheckpointNumber,
			"testMessage", req.TestMessage,
			"from", from.String(),
		)
		...
		// 验证消息
		if err := msg.ValidateBasic(); err != nil {
			helper.Logger.Error("repairCheckpointTestHandlerOptimized, 消息验证失败", "error", err)
			rest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
			return
		}

		// 使用标准的 REST 方式生成未签名交易
		helper.Logger.Info("repairCheckpointTestHandlerOptimized, 生成未签名交易",
			"checkpointNumber", req.CheckpointNumber,
			"testMessage", req.TestMessage,
			"from", from.String(),
		)
